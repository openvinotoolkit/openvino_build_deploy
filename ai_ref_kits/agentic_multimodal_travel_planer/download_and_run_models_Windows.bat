...
powershell -Command "Start-Process -FilePath '%OVMS_PATH%' -ArgumentList '%LLM_ARGS%' -RedirectStandardOutput '%LOGS_DIR%\ovms_llm.log' -RedirectStandardError '%LOGS_DIR%\ovms_llm.log' -WindowStyle Hidden" || (echo Failed to start LLM service && exit /b 1)
ping -n 3 127.0.0.1 >nul
...
REM Use PowerShell Start-Process to launch detached
powershell -Command "Start-Process -FilePath '%OVMS_PATH%' -ArgumentList '%VLM_ARGS%' -RedirectStandardOutput '%LOGS_DIR%\ovms_vlm.log' -RedirectStandardError '%LOGS_DIR%\ovms_vlm.log' -WindowStyle Hidden" || (echo Failed to start VLM service && exit /b 1)
ping -n 3 127.0.0.1 >nul
