...
def draw_spooky(img, poses, faces, landmarks):
    ...
    for pose in poses:
        points, scores = pose[:, :2].astype(np.int32), pose[:, 2]
        for i, j in DEFAULT_SKELETON:
            if i < len(scores) and j < len(scores) and scores[i] > 0.25 and scores[j] > 0.25:
...
def draw_easter(img, poses, faces, landmarks):
    for pose in poses:
        ...
        if torso_center is not None:
            draw_mask(img, egg_img, torso_center, (100, 150), scale=1.0)
    for (_, box), lm in zip(faces, landmarks):
...
def run_demo(source, theme, model_precision, device, flip):
    ...
    # Debug model expectation
    expected_shape = list(pose_model.input(0).shape)
    ...
    while True:
        ...
        print(f"Input shape to pose model: {pose_input.shape}")  # Debug line
