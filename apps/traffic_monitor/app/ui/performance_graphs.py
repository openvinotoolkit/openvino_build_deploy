...
class PerformanceGraphsWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        # Set golden color scheme for latency graph (no red/green boundaries)
        self.latency_graph.line_color = QColor(255, 215, 0)      # Golden line
        self.latency_graph.spike_color = QColor(255, 223, 0)     # Light golden for spikes  
        self.latency_graph.cpu_color = QColor(255, 215, 0)       # Golden for CPU markers
        self.latency_graph.gpu_color = QColor(255, 215, 0)       # Golden for GPU markers
        self.latency_graph.accent_color = QColor(255, 215, 0)    # Golden accent
        
        latency_layout.addWidget(self.latency_graph)
        
        # Latency statistics - vertical layout
        stats_container = QWidget()
        stats_layout = QVBoxLayout(stats_container)
        stats_layout.setContentsMargins(8, 4, 8, 4)
        stats_layout.setSpacing(2)
        
        self.latency_max_label = QLabel("Max: 0ms")
        self.latency_min_label = QLabel("Min: 0ms") 
        self.latency_avg_label = QLabel("Avg: 0ms")
        self.latency_spike_label = QLabel("Spikes: 0")
        ...
        self.latency_max_label.setStyleSheet(stat_style)
        self.latency_min_label.setStyleSheet(stat_style)
        self.latency_avg_label.setStyleSheet(stat_style)
        self.latency_spike_label.setStyleSheet(stat_style)
        
        stats_layout.addWidget(self.latency_max_label)
        stats_layout.addWidget(self.latency_min_label)
        stats_layout.addWidget(self.latency_avg_label) 
        stats_layout.addWidget(self.latency_spike_label)
        
        latency_layout.addWidget(stats_container)
        ...
        # FPS statistics - vertical layout
        fps_stats_container = QWidget()
        fps_stats_layout = QVBoxLayout(fps_stats_container)
        fps_stats_layout.setContentsMargins(8, 4, 8, 4)
        fps_stats_layout.setSpacing(2)
        
        self.fps_current_label = QLabel("Current FPS: 0")
        self.fps_resolution_label = QLabel("Resolution: -")
        self.fps_device_label = QLabel("Device: -")
        self.fps_model_label = QLabel("Model: -")
        ...
        self.fps_current_label.setStyleSheet(fps_stat_style)
        self.fps_resolution_label.setStyleSheet(fps_stat_style)
        self.fps_device_label.setStyleSheet(fps_stat_style)
        self.fps_model_label.setStyleSheet(fps_stat_style)
        
        fps_stats_layout.addWidget(self.fps_current_label)
        fps_stats_layout.addWidget(self.fps_resolution_label)
        fps_stats_layout.addWidget(self.fps_device_label)
        fps_stats_layout.addWidget(self.fps_model_label)
        
        fps_layout.addWidget(fps_stats_container)
        ...
        # Device statistics - vertical layout
        device_stats_container = QWidget()
        device_stats_layout = QVBoxLayout(device_stats_container)
        device_stats_layout.setContentsMargins(8, 4, 8, 4)
        device_stats_layout.setSpacing(2)
        
        self.cpu_usage_label = QLabel("CPU: 0%")
        self.ram_usage_label = QLabel("RAM: 0%")
        self.gpu_status_label = QLabel("GPU: Available")
        ...
        self.cpu_usage_label.setStyleSheet(device_stat_style)
        self.ram_usage_label.setStyleSheet(device_stat_style)
        self.gpu_status_label.setStyleSheet(device_stat_style)
        
        device_stats_layout.addWidget(self.cpu_usage_label)
        device_stats_layout.addWidget(self.ram_usage_label)
        device_stats_layout.addWidget(self.gpu_status_label)
        
        device_layout.addWidget(device_stats_container)
    ...
    def _update_latency_graph(self, real_time_data: dict, current_metrics: dict, current_time: float):
        ...
        try:
            ...
            if latency_values:
                ...
                if hasattr(self, 'latency_max_label') and self.latency_max_label:
                    self.latency_max_label.setText(f"Max: {self.latency_stats['max']:.1f}ms")
                if hasattr(self, 'latency_min_label') and self.latency_min_label:
                    self.latency_min_label.setText(f"Min: {self.latency_stats['min']:.1f}ms")
                if hasattr(self, 'latency_avg_label') and self.latency_avg_label:
                    self.latency_avg_label.setText(f"Avg: {self.latency_stats['avg']:.1f}ms")
                if hasattr(self, 'latency_spike_label') and self.latency_spike_label:
                    self.latency_spike_label.setText(f"Spikes: {self.latency_stats['spike_count']}")
    ...
    def _update_fps_graph(self, real_time_data: dict, current_metrics: dict, current_time: float):
        ...
        try:
            ...
            if fps_values:
                ...
                # Update individual FPS stat labels
                model_name = current_metrics.get('model', 'Unknown')
                if hasattr(self, 'fps_current_label') and self.fps_current_label:
                    self.fps_current_label.setText(f"Current FPS: {latest_fps:.1f}")
                if hasattr(self, 'fps_resolution_label') and self.fps_resolution_label:
                    self.fps_resolution_label.setText(f"Resolution: {resolution}")
                if hasattr(self, 'fps_device_label') and self.fps_device_label:
                    self.fps_device_label.setText(f"Device: {device}")
                if hasattr(self, 'fps_model_label') and self.fps_model_label:
                    self.fps_model_label.setText(f"Model: {model_name}")
    ...
    def update_system_metrics(self):
        ...
        try:
            ...
            # Update individual device stat labels
            if hasattr(self, 'cpu_usage_label') and self.cpu_usage_label:
                self.cpu_usage_label.setText(f"CPU: {cpu_percent:.1f}%")
            if hasattr(self, 'ram_usage_label') and self.ram_usage_label:
                self.ram_usage_label.setText(f"RAM: {ram_percent:.1f}%")
            if hasattr(self, 'gpu_status_label') and self.gpu_status_label:
                self.gpu_status_label.setText("GPU: Available")
                
        except Exception as e:
            print(f"[PERF GRAPHS ERROR] System metrics update failed: {e}")
            # Fallback in case of any error
            if hasattr(self, 'cpu_usage_label') and self.cpu_usage_label:
                self.cpu_usage_label.setText("CPU: --")
            if hasattr(self, 'ram_usage_label') and self.ram_usage_label:
                self.ram_usage_label.setText("RAM: --")
            if hasattr(self, 'gpu_status_label') and self.gpu_status_label:
                self.gpu_status_label.setText("GPU: Error")
