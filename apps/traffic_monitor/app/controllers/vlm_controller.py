...
class VLMControllerThread(QThread):
    ...
    def __init__(self, vlm_dir=None):
        ...
        if vlm_dir is None:
            ...
            self.vlm_dir = current_dir / "mistral_openvino_model"
        else:
    ...
    def run(self):
        ...
        while not self.abort:
            ...
            try:
                ...
                print(f"[VLM DEBUG] Failed to emit result_ready: {e}")
    ...
    def _load_model(self):
        ...
        try:
            ...
            if not self.vlm_dir.exists():
                print(f"[VLM DEBUG] ❌ VLM model directory not found. Please download the model first.")
                print(f"[VLM DEBUG] 💡 Expected directory: {self.vlm_dir.name} (relative path)")
                return
            ...
            # Check for required OpenVINO files (expected names)
            required_files = [
            ...
            try:
                print(f"[VLM DEBUG] 🚀 Loading VLMPipeline from: {self.vlm_dir.name} (relative path)")
    ...
    def process_request(self, image, prompt):
        """Process a VLM request (thread-safe setter)."""
        self.mutex.lock()
        try:
            self.image = image
            self.prompt = prompt
    ...
    def _run_genai_inference(self, image: np.ndarray, prompt: str) -> dict:
        ...
        try:
            ...
            print(f"[VLM DEBUG] 🖼️ Image shape: {getattr(image, 'shape', None)}")
            ...
            # Convert PIL image to numpy and NCHW
            image_array = np.array(pil_image)
            # Ensure NCHW format for OpenVINO when required by model
            if len(image_array.shape) == 3:
                image_array = np.transpose(image_array, (2, 0, 1))  # HWC to CHW
            image_array = np.expand_dims(image_array, axis=0)  # Add batch dimension
            
            # Create OpenVINO Tensor (sanity)
            try:
                image_tensor = ov.Tensor(image_array)
                print(f"[VLM DEBUG] 🔢 Image tensor shape: {image_tensor.shape}")
            except Exception as e:
                print(f"[VLM DEBUG] ⚠️ Failed to create ov.Tensor: {e}")
                # Some OpenVINO versions accept numpy array directly; pass numpy array instead
                image_tensor = image_array
            
            # Start chat session and run generation
            print(f"[VLM DEBUG] 💬 Starting chat session...")
            try:
                self.vlm_pipeline.start_chat()
            except Exception as e:
                print(f"[VLM DEBUG] ⚠️ start_chat() failed: {e}")
...
class VLMController(QObject):
    ...
    def __init__(self, vlm_dir=None):
        ...
        if vlm_dir is None:
            current_dir = Path(__file__).parent.parent
            vlm_dir = current_dir / "mistral_openvino_model"
        else:
            vlm_dir = Path(vlm_dir).resolve()
