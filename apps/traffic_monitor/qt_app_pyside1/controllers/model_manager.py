    ...
    def _find_model_path(self, model_name):
        ...
        if cwd_openvino_dir.exists():
            ...
            if direct_path.exists():
                print(f"✅ Found {model_name} model directly in CWD: {direct_path}")
        ...
        if absolute_openvino_dir.exists():
            direct_path = absolute_openvino_dir / f"{model_name}.xml"
            if direct_path.exists():
                print(f"✅ Found {model_name} model at absolute path: {direct_path}")
                return str(direct_path.absolute())
        ...
        if subfolder_path.exists():
            print(f"✅ Found {model_name} model in subfolder: {subfolder_path}")
        ...
        search_dirs = [
            ...
            "../rcb",
            "../openvino_models",
            f"../{model_name}_openvino_model",
            "../..",  # Go up to project root
            "../../openvino_models",  # Project root / openvino_models
        ]
        
        model_extensions = [
            (f"{model_name}.xml", "OpenVINO IR direct"),
            (f"{model_name}_openvino_model/{model_name}.xml", "OpenVINO IR"),
            (f"{model_name}.pt", "PyTorch"),
        ]
        ...
        for search_dir in search_dirs:
            ...
            for model_file, model_type in model_extensions:
                ...
                if model_path.exists():
                    print(f"✅ Found {model_name} {model_type} model: {model_path}")
                    return str(model_path.absolute())
        
        print(f"❌ No {model_name} model found")
        return None
